...
**Goal:** Redesign `app/blog/[slug]/page.tsx` to a two-column layout with a sticky sidebar (author card, interactive TOC, compact related articles) matching the provided mockup.

**Architecture:** Server Component page + one Client Component (`TableOfContents`) for smooth-scroll interactivity. Headings extracted from raw markdown server-side via regex; the same `slugify` function adds matching `id` attributes to `h2` elements in ReactMarkdown.

**Tech Stack:** Next.js App Router (RSC), React, Tailwind CSS v4, ReactMarkdown + remark-gfm, lucide-react (`X`, `Link2`), inline SVGs for LinkedIn/Facebook

---

### Task 1: Create `components/table-of-contents.tsx`
...
```bash
git add components/table-of-contents.tsx
git commit -m "feat: add TableOfContents client component with smooth scroll"
```

---

### Task 2: Rewrite `app/blog/[slug]/page.tsx`
...
Open `http://localhost:3000/blog/<any-slug>` and check:
- Two-column layout appears on desktop (≥ 1024px)
- Sidebar is hidden on mobile (< 1024px)
- Breadcrumb shows: Acasă > Articole > [title]
- TOC items scroll to the correct heading on click
- Author card shows avatar, name, role, bio, link button
- Related articles show in sidebar (compact format)
- Bottom related articles section is gone

- [ ] **Step 3: Commit**

```bash
git add app/blog/[slug]/page.tsx
git commit -m "feat: redesign blog post page with two-column layout and sticky sidebar"
```
